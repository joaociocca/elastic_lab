...
### Escalando o Elasticsearch

Fui então perguntar pro [Pai dos Burros Digitais, "como rodar múltiplas instâncias similares no Docker?"](https://www.google.com/search?q=docker+how+to+run+multiple+similar+services) e um dos resultados foi essa explicação rápida do [PSPDFKit](https://pspdfkit.com/blog/2018/how-to-use-docker-compose-to-run-multiple-instances-of-a-service-in-development/). Ajudou, mas não foi a bala de prata porque aqui rola um problema que eles não tem: volumes individuais pra cada instância. Afinal, ES1-01 tem dados diferentes em seu volume do que o ES1-02, certo?

Um pouco mais de pesquisas, e o [Pai dos Devs Burros me mostrou uma solução](https://stackoverflow.com/a/56209034/1985023)! Vou tentar aplicar isso pra reduzir o docker-compose atual e ainda poder ter 3 nós do Elasticsearch.

Claro que aí a gente tá aumentando a complexidade da coisa, porque vamos precisar adicionar um NGINX na brincadeira pra lidar com as portas dinâmicas do Elasticsearch... e tem que pensar numa maneira de fazer a criação dinâmica dos certificados para TLS (das senhas não é necessário, pois o keystore já é um volume mapeado para persistência).

Mas esse papo todo me fez pensar numa outra coisa: "Como os nós de Elasticsearch vão se descobrir?"

Uma coisa é quando temos todos os nós definidos previamente. Dá pra listar todos na configuração `discovery.seed_hosts` e resolvido tudo. Mas pra passar pra essa configuração, não rola. Mano, que treta gigante que é isso! Mas o lado bom é que conseguindo isso, a gente já aprende como preparar um ambiente que possa ESCALAR #instantRimShot

Parece que o jeito vai ser usar o ["File-based seed hosts provider"](https://www.elastic.co/guide/en/elasticsearch/reference/current/modules-discovery-hosts-providers.html#file-based-hosts-provider). Até onde eu sei, dá pra colocar no `docker-compose.yml` um comando a ser executado quando o container sobe... então é só adicionar algo lá que faça um `echo ip >> $conf/unicast_hosts.txt`. Mas será que dá pra fazer também um comando que remova o IP quando o container for derrubado..?

Mais uma vez, o StackOverflow vem me salvar - dá pra fazer usando um script de ENTRYPOINT que capture o SIGTERM pra executar algo. Talvez assim dê pra fazer um `grep -v ip $conf/unicast_hosts > $conf/unicast_hosts.txt`...? Quanta coisa pra fazer.

Parecia complicado... não lembro mais o que eu estava procurando, mas esbarrei [nesse outro site](https://kgiann78.github.io/docker/elasticsearch/2016/12/17/Docker-using-compose-to-scale-up-elasticsearch-cluster.html). Ele pareceu deixar as coisas bem mais simples pra fazer isso, certo? Não parece ser o caso. Esse modo "stack" parecia ser bem mais simples, mas parece que complica muitas outras coisas. Tive que fazer muitas alterações que ainda nem entendo direito no `docker-compose.yml`... cheguei a um ponto em que o comando `docker stack deploy -c docker-compose.yml el` parece funcionar pro Elasticsearch e Kibana, mas não pro Filebeat. Além disso, parece que os containers não estabilizam e ficam sendo reiniciados o tempo todo.

Vou deixar esse branch "stack_tests" aqui do lado, quem sabe mais adiante se/quando eu entender melhor o Docker eu volto nele. Por enquanto... foram uns 2 dias de tentativas por essa outra linha de pensamento que deu num beco sem saída.
